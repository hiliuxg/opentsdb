      ...
      public Object call(final ArrayList<DataPoints[]> query_results)
        ...
        class SendIt implements Callback<Object, ChannelBuffer> {
          ...
          public Object call(final ChannelBuffer buffer) throws Exception {
            //进行中文编码转化
            String decodeJson = UnicodeUtil.decodeString(buffer.toString(Charset.forName("UTF-8")));
            if (decodeJson != null) {
              ChannelBuffer decodeBuffer = ChannelBuffers.copiedBuffer(decodeJson.getBytes());
              query.sendReply(decodeBuffer);
            }else{
